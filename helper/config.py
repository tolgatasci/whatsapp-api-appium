...
class Config:
    ...
    WHATSAPP_APP_FILE_URL_32_64 = "http://www.remote.com/whatsapp_32_64.apk"
    def __init__(self):
        pass
    ...
    MESSAGES_ROW_REGEX = b"(?P<_id>\d+)\|(?P<key_remote_jid>.*?)\|(?P<key_from_me>\d+)\|(?P<key_id>.*?)\|(?P<status>\d+)\|(?P<data>[\s\S]+?)\|(?P<timestamp>\d+)|(?P<read_device_timestamp>\d+)|(?P<last_message_row_id>\d+)|(?P<bizim_id>\d+)\|\n"
